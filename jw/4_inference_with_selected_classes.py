...
from src.vis import vis_ms_att, vis_att
...
if __name__ == '__main__':
    ...
    for i, batch_data in enumerate(tqdm(test_loader)):
        ...
        os.makedirs(os.path.join(figure_dir, f"batch_{strt}"), exist_ok=True)
        for i in range(bs):
            ...
            if not fig_label.nonzero().shape[0] == 0:    
                for nonzero_i in range(fig_label.nonzero().shape[0]):
                    ...
                    fig_label_path = os.path.join(figure_dir, f"batch_{strt}", f"label_{i}_nonzero{nonzero_i}.png")
                    ms_att_map = {s: att_maps[s][i][unseen_id] for s in opt.scales}
                    vis_ms_att(raw_img_path, fig_label_path, opt.scales, ms_att_map, class_name)
                ...
                for topk_i in range(num_k):
                    ...
                    fig_label_path = os.path.join(figure_dir, f"batch_{strt}", f"pred_{i}_topk{topk_i}.png")
                    ms_att_map = {s: att_maps[s][i][unseen_id] for s in opt.scales}
                    vis_ms_att(raw_img_path, fig_label_path, opt.scales, ms_att_map, class_name)
