...
if __name__ == '__main__':
    ...
    model_joint_emb = model.StandardJE(vecs=vecs_7186)
 
 
 
 
    
    # model_BiAM = models.BiAM(opt, dim_feature=[196,512])
    if opt.cuda:
        model_joint_emb = model_joint_emb.cuda()
    ...
    for epoch in range(start_epoch, num_epochs):
        for chunk_id in range(len(chunk_datasets)):
            ...
            for i, batch_data in enumerate(chunk_dataloader):
                ...
                logits = model_joint_emb(ms_train_inputs[1])
            ...
            if (chunk_id > 3 and chunk_id % 100 == 0):
                torch.save(model_joint_emb.state_dict(), os.path.join(opt.save_path, f"model_latest_epoch{epoch}_chunk{chunk_id}.pth"))
